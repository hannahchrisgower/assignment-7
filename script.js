...
let favoritefoods = ["Sushi", "Apples", "Fried Pickles", "Bao Buns", "Chocolate", "Baked Ziti"]
...
// ...etc.
for (let i = 0; i < favoritefoods.length; i++) {
  console.log("My #" + (1+i) + " favorite food is " + favoritefoods[i]);
}
...
printFoodRecommendation("Baked Ziti")
printFoodRecommendation("Pad Thai")
...
let candidateFood = ""
for (let i = 0; i < friendFavorites.length; i++) {
  candidateFood = friendFavorites[i]
  if (candidateFood.length > longestFood.length) {
